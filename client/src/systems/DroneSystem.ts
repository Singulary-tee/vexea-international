...
export class DroneSystem {
  ...
  public step(dt: number) {
    ...
    match.droneJitterMap.forEach((buffer, id) => {
      if (!buffer || typeof buffer.get !== 'function' || buffer.count === 0) return;
                      ...
                      for(let n=0; n<nodesInfo.length; n++) {
                         ...
                         if (info.parentName) {
                             if (state.processedNodes.has(info.parentName)) {
                                 const pMat = state.droneLocalMats ? state.droneLocalMats.get(info.parentName) : null;
                                 if (pMat) parentDroneLocalMat = pMat;
                             } else {
                                 unresolved = true;
                                 continue;
                             }
                         ...
                         if (!state.droneLocalMats) state.droneLocalMats = new Map<string, THREE.Matrix4>();
