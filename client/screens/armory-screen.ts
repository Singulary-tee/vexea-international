...
const ARMORY_ICON_STYLE_ID = 'vexea-armory-file-icons';
function ensureArmoryIconStyle(): void {
  if (document.getElementById(ARMORY_ICON_STYLE_ID)) return;
  const style = document.createElement('style');
  style.id = ARMORY_ICON_STYLE_ID;
  style.textContent = `.armory-slot-icon { width: 1.13rem; height: 1.13rem; object-fit: contain; opacity: 0.85; pointer-events: none; user-select: none; display: block; }`;
...
export function renderArmoryScreen(container: HTMLElement, registeredUserData?: any): void {
  ...
  // Class identity is shown on the loadout content surface; class navigation remains text-only.
  const classIdentity = document.createElement('div');
  classIdentity.innerHTML = `<img class="ui-svg-icon armory-class-icon" src="/ui_svgs/class_${activeCategory.toLowerCase()}.svg" alt="" aria-hidden="true" draggable="false"><span>${activeCategory === 'DEMOLITIONS' ? 'DEMOLITIONS' : activeCategory} LOADOUT</span>`;
  Object.assign(classIdentity.style, {
    display: 'flex', alignItems: 'center', gap: '6px', minHeight: '20px',
    fontFamily: DS.typography.fontFamily, fontSize: DS.typography.sizes.tiny,
    fontWeight: 'bold', letterSpacing: '0.8px', color: '#9aa2ae'
  });
  leftCol.appendChild(classIdentity);

function getSlotIconSvg(weaponKey: string, category: string, itemName: string): string {
  const key = weaponKey.toLowerCase();
  const cat = category.toLowerCase();
  const name = itemName.toLowerCase();
  const asset = (() => {
    if (key === 'rifle' || cat.includes('rifle')) return 'rifle';
    if (key === 'pistol' || cat.includes('sidearm') || cat.includes('pistol')) return 'pistol';
    if (name.includes('flash')) return 'utility_flashbang';
    if (name.includes('frag') || key.includes('grenade') || cat.includes('grenade')) return 'utility_grenade';
    if (name.includes('medkit') || name.includes('med kit') || cat.includes('health')) return 'medkit';
    if (name.includes('revive')) return 'utility_revive';
    if (name.includes('jammer') || name.includes('disruptor') || name.includes('emp')) return 'utility_jammer';
    if (name.includes('radio') || cat.includes('comms') || cat.includes('electronic') || key.includes('radio')) return 'radio';
    if (name.includes('c4') || cat.includes('c4')) return 'utility_c4';
    if (name.includes('mine') || cat.includes('mine')) return 'utility_mine';
    return 'utility_grenade';
  })();
  return `<img class="ui-svg-icon armory-slot-icon" src="/ui_svgs/${asset}.svg" alt="" aria-hidden="true" draggable="false">`;
}
  ...
  currentList.forEach((item, idx) => {
    ...
    itemCard.innerHTML = `
      <div style="display: flex; align-items: center; justify-content: center; width: 1.13rem; height: 1.13rem; color: ${isSelected ? '#FF4500' : '#888888'};">
        ${getSlotIconSvg(item.weaponKey, item.category, item.name)}
      </div>
