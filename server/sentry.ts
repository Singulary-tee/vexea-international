...
export async function initSentry(): Promise<void> {
  const isEnabled = await serverFlagService.getBoolean(FeatureFlagKey.SENTRY_SERVER_ENABLED, undefined, true);
  if (!isEnabled) {
  ...
  const tracesSampleRate = await serverFlagService.getNumber(FeatureFlagKey.SENTRY_SERVER_TRACES_RATE, undefined, 1.0);
  const enableProfiling = await serverFlagService.getBoolean(FeatureFlagKey.SENTRY_NODE_PROFILING, undefined, true);
  const enableLLMTracing = await serverFlagService.getBoolean(FeatureFlagKey.SENTRY_LLM_TRACING, undefined, true);
...
export async function recordServerTickDuration(durationMs: number): Promise<void> {
  const enabled = await serverFlagService.getBoolean(FeatureFlagKey.SENTRY_SERVER_METRICS_ENABLED, undefined, true);
...
export async function recordSecurityExploit(exploitType: string, details: Record<string, unknown>): Promise<void> {
  const loggingEnabled = await serverFlagService.getBoolean(FeatureFlagKey.SECURITY_EXPLOIT_LOGGING, undefined, true);
  ...
  const metricsEnabled = await serverFlagService.getBoolean(FeatureFlagKey.SENTRY_SERVER_METRICS_ENABLED, undefined, true);
  if (metricsEnabled) {
