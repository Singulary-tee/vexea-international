...
const BASE_URL = 'https://api.configcat.com/v2';
...
const FLAGS_TO_SYNC: FlagDef[] = [
  ...
  { key: 'llm_fallback_models', scope: 'SERVER', type: 'array', defaultValue: JSON.stringify(['gemini-3.6-flash', 'gemini-3.1-flash']), purpose: 'Fallback Gemini models on rate limits.' },
  ...
  { key: 'kimi_fallback_models', scope: 'SERVER', type: 'array', defaultValue: JSON.stringify(['kimi-k2.5']), purpose: 'Fallback Kimi models on rate limits.' },
  ...
  { key: 'claude_fallback_models', scope: 'SERVER', type: 'array', defaultValue: JSON.stringify(['claude-opus-4-8', 'claude-haiku-4-5-20251001']), purpose: 'Fallback Claude models on rate limits.' },
  ...
  { key: 'openai_fallback_models', scope: 'SERVER', type: 'array', defaultValue: JSON.stringify(['gpt-5.6-terra']), purpose: 'Fallback OpenAI models on rate limits.' },
...
async function sync() {
  ...
  for (const product of products) {
    ...
    for (const config of configs) {
      ...
      for (const flag of FLAGS_TO_SYNC) {
        ...
        // Now set default values across environments if environment exists
        for (const env of environments) {
          try {
            await apiRequest(`/environments/${env.environmentId}/settings/${settingId}/value`, 'PUT', {
              value: flag.defaultValue,
            });
            console.log(`    Updated '${flag.key}' in env '${env.name}' -> ${JSON.stringify(flag.defaultValue)}`);
          } catch (err: any) {
