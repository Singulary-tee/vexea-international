...
### Cycle 2026-08-27-02: Doppler Client-Secrets Fail-Fast (No-Token Boot Noise Removal)
*   **Target Files:** `server/routes/api-routes.ts`, `client/doppler.ts`, `CODEBASE_INDEX.md`
*   **Status:** Verified on working tree of codespace `supreme-space-train-7vr496j4wwxxfx5p7` (branch `agent/menu-modularization-1`); awaiting manual merge review.
*   **Modifications:**
    *   `server/routes/api-routes.ts` (GET /api/doppler-client-secrets): when no Doppler token is configured in the environment, responds `200 {available:false}` instead of `400 {error}`. Token-present behavior unchanged (proxy fetch + pass-through, upstream error passthrough preserved).
    *   `client/doppler.ts`: added module-level negative cache `proxyUnavailable`. A non-OK response or an `available:false` marker sets it once; subsequent `loadClientDopplerSecrets()` calls short-circuit for the session. Recognizes the availability marker before treating the payload as a secrets map. Secret resolution order (`getClientDopplerSecret` env fallbacks) intact.
    *   Rationale: on environments without Doppler (e.g., codespace with empty DOPPLER_TOKEN), every boot produced a guaranteed 400 + client warn; this converts it to a single informational flow with no error-shape network traffic.
*   **Verification:** `tsc --noEmit` 0 errors; full Vitest suite 107/107 passed; dev server restarted and direct endpoint check returns `200 {"available":false}`; full Playwright E2E probe re-run (gate 13.9s -> menu 22.0s -> lobby -> back to menu) with the `/api/doppler-client-secrets` 400 completely absent from BAD RESPONSES (remaining entries are benign ERR_ABORTED long-poll teardowns, unchanged baseline).
---
### Cycle 2026-08-27-03: Main-Menu Monolith Break-Up Phase 2 — Dev Architectural-Analysis Modal Extraction
*   **Target Files:** `client/screens/main-menu.ts`, `client/screens/dev-analysis.ts` (new), `CODEBASE_INDEX.md`
*   **Status:** Verified on working tree of codespace `supreme-space-train-7vr496j4wwxxfx5p7` (branch `agent/menu-modularization-1`); awaiting manual merge review.
*   **Modifications:**
    *   `client/screens/main-menu.ts`: Reduced from 2369 to 2281 lines (~4%). Removed the standalone Dev-only `showArchitecturalAnalysis()` single-purpose modal (formerly main-menu.ts:2228-2314, 87 lines) and imported the new module to replace it. The sole caller (the 'DEV ANALYSIS' button created in `initDevButtons`, now main-menu.ts:873) is unchanged. No re-exports required: no external module dynamic-imports `showArchitecturalAnalysis` from `main-menu`.
    *   `client/screens/dev-analysis.ts` (new): single-responsibility module owning the "ARCHITECTURAL SERVICE ANALYSIS" DEV modal. Exports `showArchitecturalAnalysis()`, body byte-identical to the original (verified via norm-diff against the pre-edit block). Imports pulled onto the module: `DS`, `isClientSentryInitialized`/`getSentryDSN`, `clientFlagService`, `ClientFeatureFlagKey`, `SharedFeatureFlagKey`. No coupling to main-menu runtime state (operates on window/document only).
*   **Verification:** `tsc --noEmit` 0 errors; full Vitest suite 21 files / 107 tests passed; dev server restarted and full Playwright E2E probe re-run (splash gate -> menu -> lobby -> back) with unchanged error profile — 0 asset-guard/origin errors, BAD RESPONSES reduced to 3 benign ERR_ABORTED long-poll teardowns, unique console/page errors identical to baseline (only the two pre-existing GPU pageerrors: popErrorScope 'Instance dropped' and the three.js menu-bg `createBuffer(6600)` headless quirk). Dedicated micro-probe confirmed the DEV ANALYSIS button opens the extracted modal: `present: true` with `[ARCHITECTURAL SERVICE ANALYSIS]` title/content and expected Doppler / ConfigCat / Sentry rows.
...
* **Status:** Verified and finalized on branch `manus/first-person-weapon-viewmodels-20260827`.
* **Verification:** `npx vitest run tests/weapon-contracts.test.ts` passed 7/7; `npx tsc --noEmit` passed; full `npm test -- --run` passed 21/21 files and 108/108 tests; architectural guards confirmed no forbidden renderer/React changes and retained Geckos references; `git diff --check` passed; separate server `esbuild` bundle passed with the repository’s existing CJS `import.meta` warning. The configured `npm run build` and quiet Vite client build both reached `rendering chunks (7)` and were terminated with status 143 by the sandbox before completion; this is recorded as an environment verification limitation, not a passing build claim.
* **Visual Evidence:** `/tmp/vexea-viewmodel-visual-findings.md`, `/tmp/vexea-viewmodel-evidence/ump-hip-frame001.png`, `/tmp/vexea-viewmodel-evidence/ump-ads-frame001.png`, `/tmp/vexea-viewmodel-evidence/ump-idle-frame001.png`, `/tmp/vexea-viewmodel-evidence/ump-fire-frame012.png`, `/tmp/vexea-viewmodel-evidence/ump-reload-frame030.png`, `/tmp/vexea-viewmodel-evidence/scar-l-shared-yaw-hip.png`. Textured UMP GLB import decoded all 13 Draco primitives and all nine authored clips; semantic marker probes verified forward/up/magazine directions. UMP idle/fire/reload and three-quarter hierarchy renders passed weapon-only review. The SCAR-L diagnostic was recorded as insufficiently lit and is not claimed as accepted visual proof.

### Cycle 2026-08-28-01: Mixamo Player Replacement and Equipped Humanoid Pose (Verified)
* **Target Files:** `client/model-manifest.ts`, `shared/asset-structure.ts`, `shared/constants.ts`, `tests/weapon-contracts.test.ts`, and `CODEBASE_INDEX.md`. `client/drone_models.ts` and `client/src/systems/DroneSystem.ts` were audited but did not require code changes because their existing zero-animation behavior already leaves static humanoid clones unchanged.
* **Scope:** Build and optimize the topology-compatible Mixamo-rigged player GLB with preserved source textures and available Mixamo rifle/pistol actions; replace the canonical player R2 object; preserve and optimize the exact saved `hPumanoidNOANIMTRAFK.glb` F90-equipped pose; explicitly represent the humanoid as a static `hold` presentation rather than claiming absent gameplay animation tracks.
* **Explicit Non-Scope:** Do not strip textures, alter weapon/audio/image/VFX/SVG domains, remove Geckos.io, change renderer architecture, or resume third-person grip experimentation. Existing source GLBs remain untouched. The player asset remains a character/animation replacement and does not claim authored third-person weapon grips. The humanoid replacement is intentionally static; movement/shoot/reload animation authoring remains unbuilt.
* **Player Artifact Evidence:** The final Draco/WebP GLB is 1,878,420 bytes, with one mesh, one skin, two materials, five embedded WebP images, and 24 animations: four `rifle_*` and twenty `pistol_*`. Every action has 123 channels targeting 41 existing Mixamo skeleton nodes, with no missing-node targets. The direct and isolated-action textured renders are `/tmp/vexea-player-evidence/player-contact-sheet.png` and `/tmp/vexea-player-draco-evidence/player-contact-sheet.png`.
* **Humanoid Artifact Evidence:** The final pose-preserving Draco/lossless-WebP GLB is 7,973,004 bytes, with 149 nodes, four meshes, two skins, four materials, twelve embedded WebP images, and zero animations. Its node matrices were compared before/after optimization with zero changed nodes. Four-view direct-source and optimized renders are `/tmp/vexea-humanoid-source-evidence/humanoid-source-contact-sheet.png` and `/tmp/vexea-humanoid-draco-evidence/humanoid-draco-contact-sheet.png`.
...
### Cycle 2026-09-18-01: Authored Facility GLB Scene Integration (Recovered, Unverified In-Browser)
* **Target Files:** `client/public/assets/maps/map_1/facility-v3.glb`, `shared/maps/map_1_facility.spec.json`, `client/src/map/MapLoader.ts`, `docs/vexea/MAP_INTEGRATION_PLAN_2026-09-18.md`, `docs/vexea/MAP_INTEGRATION_STATUS_2026-09-18.md`, `CODEBASE_INDEX.md`.
* **Scope:** Render a real authored map for a demo gameplay video. Add a top-level `sceneGlb` field to the facility map spec; load that GLB once as a scene-level asset rather than per building; place the origin-centered authored geometry at the spec world center `(wX/2, 0, wZ/2)`; bypass the procedural centerpiece when a scene GLB is present; port the authoring viewer's ground micro-variation bake (deterministic LCG-seeded value-noise canvas albedo and roughness maps) and its overcast lighting rig (raking sun, hemisphere fill, `FogExp2`, warm asphalt ground) into `setupEnvironment()`; expand the ground plane to `max(wX,2200) x max(wZ,1800)` and drop it to `Y=-0.05` to avoid z-fighting with authored ground-level geometry; dispose the new canvas textures in `dispose()`.
* **Explicit Non-Scope:** No server, physics, zone, spawn-point, or networking changes. Colliders remain derived from the spec `buildings[]` array and are approximate against the visible GLB mesh. No `map-registry.ts` change was required because `assetDirectory` already resolves to the copy destination. No shadow-map enablement, IBL substitution, or tone-mapping change was made. No character pose work.
* **Provenance:** These edits were produced by an interrupted agent session (upstream `RESOURCE_EXHAUSTED 429`) and were left uncommitted in the working tree. This cycle registers the recovered work; it does not claim authorship by the recovering agent. The original plan is preserved verbatim at `docs/vexea/MAP_INTEGRATION_PLAN_2026-09-18.md`, and the open-items list at `docs/vexea/MAP_INTEGRATION_STATUS_2026-09-18.md`.
* **Verification:** `npx tsc --noEmit` on the codespace (`supreme-space-train-7vr496j4wwxxfx5p7`, node v24.14.0) exited `0` with zero diagnostics against this branch. **The recovery found and fixed one build-breaking error that the interrupted session had incorrectly reported as passing:** `client/src/map/MapLoader.ts:366` called `.getHex()` on the numeric literal `0xa8bdd2` (`error TS2339: Property 'getHex' does not exist on type '11058642'`) instead of on the `SKY` colour; it now reads `SKY.getHex()`. The local VM cannot run this typecheck (it exhausts its heap at ~445 MB on an 896 MB box), so the gate was run on the codespace build arm. The GLB asset path was verified by inspection: `MAP_REGISTRY` declares `assetDirectory: 'client/public/assets/maps/map_1/'`, `MapLoader.load()` strips the `client/public/` prefix to form the browser URL, and the fetch runs through `getCachedOrFetchUrl(..., 'Asset')` under a `try/catch` that falls back to the direct public URL, so no manifest entry is required for the asset to load. **The integration has not been rendered in a browser**, and no build, test-suite, or render-gate result is claimed for it.
* **Rebase:** Rebasing onto `main` @`dc8057a` conflicted in `MapLoader.ts` (3 hunks) and `CODEBASE_INDEX.md`. Two `MapLoader.ts` hunks were purely additive (upstream's `disposed` flag and its `dispose()` cleanup vs. this branch's ground-texture fields and their disposal) and both sides were kept. The third was semantic: upstream set a dark `0x181c24` linear fog in `ebc8cfb`, an unrelated RemotePlayerSystem/engine-context refactor whose message documents nothing about lighting and which also silently dropped the HDR skybox block. This branch therefore keeps its deliberate authoring-matched overcast rig while preserving upstream's hoisting of the background/fog out of the async HDR callback and its `fogNode` null-safety. The base now carries the r186 upgrade (Cycle 2026-09-10-01) rather than r184; the ported API surface (`CanvasTexture`, `FogExp2`, `HemisphereLight`, `PlaneGeometry`) is unaffected.
* **Status:** Recovered and pushed for continuation. Not verified in-browser; see the status document for the ordered remaining work.

### Cycle 2026-09-18-02: Post-Rebase Gate Verdict + Main-Hygiene Fix Branch (tsc error, package-lock r186 sync)
* **Target Files:** `benchmarks/diagnostics/measure_benchmark_ipc.ts`, `package-lock.json`, `CODEBASE_INDEX.md` — shipped on separate branch `agent/fix-main-bench-tsc-r186-lock` (commits `b6db2ae`, `697469f`, based on `main` @`dc8057a`), pushed to origin. **None of these files are touched by the map branch**; registered here because the map branch's post-rebase verification surfaced both defects.
...
* **Verification:** `tsc --noEmit` exits 0; `npx vitest run` passes 411/411 across 47 files, including the security unit, production-gate and dev-fallback suites.
* **Status:** Implemented and verified.
